...
% =========================================================================
%
%This software is to perform Coupled Dictionary Learning based Multi-contrast MRI Reconstruction that reconstructs target contrast, e.g. T1-weighted from its
% under-sampled k-space measurements with another fully-sampled contrast, e.g. T2-weighted as reference/guidance.  
...
% QB = ones(NUMROWS, NUMCOLS); % Full sampling for side information
Q.Q1B = ( QB );
...
numIter = 20; % number of outer iterations.
ErrTh_start = 0.03; % error threshold (RMSE) in the beginning, % RMSE = 10^(-PSNR_goal/20);
ErrTh_end = 0.02;  % error threshold at the end	
errNum = 20; % number of outer iterations with error threshold decreasing linearly from ErrTh_start to ErrTh_end.
...
DLMRIparams.weights = 0.5; % weights for SI
DLMRIparams.variance_Thresh = 0.1 ; % Discard those patch pairs with too small variance during training.
DLMRIparams.Psi_cx0 = outputCDL.Psi_cy(:,1:K); % initial dicts
DLMRIparams.Psi_x0 = outputCDL.Psi_y(:,1:K);
...
FILENAME = ['CDLMRI_T2Slices_Guided', SIZE, Fold, S, MaxIter, W, DATE];
...
% 	Iout1.B = uint8( 255.*abs(Iout1.B) ) ;
	...
	% set parameters
	numIter = 5;
	ErrTh_start = 0.02; % error threshold (RMSE) in the beginning, % RMSE = 10^(-PSNR_goal/20);
	ErrTh_end = 0.02;  % error threshold at the end	
	errNum = 5;
