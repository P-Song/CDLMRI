function [Iout1,param1] = CDLMRI_Radial(Img, Q, sigmai,sigma,DLMRIparams)
% =========================================================================
...
sampling_ratioA = DLMRIparams.sampling_ratio(1)  ;
numRadialsA = DLMRIparams.numRadials(1) ;
numSamplesA = DLMRIparams.numSamples(1)  ;
sampling_ratioB = DLMRIparams.sampling_ratio(2)  ;
numRadialsB = DLMRIparams.numRadials(2) ;
numSamplesB = DLMRIparams.numSamples(2)  ;


% Build forward/inverse NUFFT transform
im_size_one_dim = size(I1A,1);
NN = [im_size_one_dim,im_size_one_dim]; %image size
dims=[NN 1 1];
...
if sampling_ratioA < 1
	% Perform forward NUFFT of data (equivalent to fft2 in when working on grid)
	I2A = reshape(Q1A*IGA.*(1./repmat(reshape(sqrt(Q1A.w),[],1,1),[1,Q1A.ncoils,1])),numSamplesA*numRadialsA,[]);
	% Perform Inverse non-uniform FFT (equivalent to ifft2 in when working on grid)
	I11A=abs(reshape((Q1A'*weight(Q1A, permute(reshape(I2A, 1, [], dims(4)),[2,1,3]))),im_size_one_dim,im_size_one_dim));
else
	I11A = I1A ;
end
...
if sampling_ratioB < 1
	% Perform forward NUFFT of data (equivalent to fft2 in when working on grid)
	I2B = reshape(Q1B*IGB.*(1./repmat(reshape(sqrt(Q1B.w),[],1,1),[1,Q1B.ncoils,1])),numSamplesB*numRadialsB,[]);
	% Perform Inverse non-uniform FFT (equivalent to ifft2 in when working on grid)
	I11B=abs(reshape((Q1B'*weight(Q1B, permute(reshape(I2B, 1, [], dims(4)),[2,1,3]))),im_size_one_dim,im_size_one_dim));
else
	I11B = I1B; % fully-sampled guidance image.
end
...
for kp=1:num
    ...
    I11A=abs(double(I11A)); % I11A=I11A/(max(max(I11A)));
    IiterA=I11A;
	
	I11B=abs(double(I11B)); % I11B=I11B/(max(max(I11B)));
    IiterB=I11B;
...
%     coupled dictionary learning algorithm 
    ...
    if(op==1)
		...
		[outputCDL] = CDL_MRI_ODL_OMP(TrainData, DLMRIparams);	 % Adapt online dictionary learning for CDL.
...
%		paramOMP.eps=0.0; % squared norm of the residual should be less than 0.1
	...
	%-----------------for modal A-----------
	if sampling_ratioA < 1
		% perform consistency with measurements. Since we do not sample on the grid, 
		% this data consistency has to be performed via a gradient step and these lines need to be replaced with:
		step=1.2;
		radialX = reshape(Q1A*IGA.*(1./repmat(reshape(sqrt(Q1A.w),[],1,1),[1,Q1A.ncoils,1])),numSamplesA*numRadialsA,[]);
		spiralX = reshape(Q1A*reshape(I3nA,im_size_one_dim,im_size_one_dim).*(1./repmat(reshape(sqrt(Q1A.w),[],1,1),[1,Q1A.ncoils,1])),numSamplesA*numRadialsA,[]);
		subXY = spiralX-radialX;
		grad_f = reshape(single((Q1A'*double(weight(Q1A, permute(reshape(subXY, 1, [], 1),[2,1,3]))))),im_size_one_dim,im_size_one_dim);
		I11A=abs(I3nA-step*grad_f);
		inn2A= abs(I11A)>1;I11A(inn2A)=1;
	else
		I11A = I1A; % fully-sampled image.
	end
	...
	if sampling_ratioB < 1
		...
		% this data consistency has to be performed via a gradient step and these lines need to be replaced with:
		step=1.2;
		radialX = reshape(Q1B*IGB.*(1./repmat(reshape(sqrt(Q1B.w),[],1,1),[1,Q1B.ncoils,1])),numSamplesB*numRadialsB,[]);
		spiralX = reshape(Q1B*reshape(I3nB,im_size_one_dim,im_size_one_dim).*(1./repmat(reshape(sqrt(Q1B.w),[],1,1),[1,Q1B.ncoils,1])),numSamplesB*numRadialsB,[]);
		subXY = spiralX-radialX;
		grad_f = reshape(single((Q1B'*double(weight(Q1B, permute(reshape(subXY, 1, [], 1),[2,1,3]))))),im_size_one_dim,im_size_one_dim);
		I11B=abs(I3nB-step*grad_f);
		inn2B= abs(I11B)>1;I11B(inn2B)=1;
	else
		I11B = I1B; % fully-sampled guidance image.
	end
